...
def test_get_posts(client: TestClient, db_session: Session) -> None:
    user_id = uuid.uuid4()
    db_session.add(
        UsersORM(id=user_id, name="Test User", email="test@example.com")
    )
    db_session.commit()
...
def test_get_post(client: TestClient, db_session: Session) -> None:
    ...
    test_post = client.post("/posts/", headers=TEST_AUTH_HEADERS, json=request_body).json()
...
def test_update_post(client: TestClient, db_session: Session) -> None:
    ...
    response = client.put(f"/posts/{post_id}", headers=TEST_AUTH_HEADERS, json=update_data)
    assert response.status_code == 200
    data = response.json()
